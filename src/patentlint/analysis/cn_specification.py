...
# greedy noun-group regex captures the WHOLE clause (up to 12 CJK
# chars before the numeral) instead of just the bound noun. Empirical
# trigger: issue #29 fixture (110P000641JP派譯版-FV.DOCX) had:
#   `當使用者踩踏踏板E10` → captured `當使用者踩踏踏板` instead of `踏板`
#   `共同形成一封閉空間SP2` → captured `共同形成一封閉空間`
#   `應設置於封閉空間SP2` → captured `應設置於封閉空間`
#   `端連接一踏板E10` → captured `端連接一踏板`
#
...
# keeps those intact.
#
# Trad/Simp parity throughout. Verbs from `_CN_INTERIOR_VERB_SPLIT_TARGETS`
# trigger the split when followed by non-suffix chars; nouns from
# `_CN_NOUN_SUFFIX_PRESERVE` block the split. Listed longest-first so
# 3-char compounds (e.g. `對應於`) match before their 2-char prefixes.
_CN_INTERIOR_VERB_SPLIT_TARGETS: tuple[str, ...] = tuple(sorted(
    (
        # Issue #29 originals:
        "踩踏",
        "連接", "连接",
...
# Chars immediately after a verb compound that mean the verb is part
# of a compound noun (don't split). Element-morpheme suffixes common
# to TIPO + CNIPA drafting. Trad/Simp pairs included throughout.
_CN_NOUN_SUFFIX_PRESERVE: frozenset[str] = frozenset({
    # Generic component / part / body suffixes
    "器", "件", "體", "体", "置", "料", "部", "管",
    "塊", "块", "層", "层", "座", "段", "盤", "盘",
    # Geometric / surface suffixes
    "板", "孔", "槽", "面", "壁", "蓋", "盖", "圈",
    "環", "环", "套", "桿", "杆", "腔", "室",
    "殼", "壳", "架",
